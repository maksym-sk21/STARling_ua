...
class TeacherCalculation(models.Model):
    ...
    def save(self, *args, **kwargs):
        super().save(*args, **kwargs)
        teacher = self.turnover.teacher
        teacher_calculations = TeacherCalculation.objects.filter(turnover__teacher=teacher)

        total_salary = sum(tc.salary_teacher for tc in teacher_calculations)

        teacher.salary = total_salary
        teacher.save()
